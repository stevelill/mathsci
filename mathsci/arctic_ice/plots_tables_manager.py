...
from mathsci.arctic_ice import rolling_predictions
...
def create_all_plots_tables(save=False, months=None):
    create_data_plots_tables(save=save)
    create_estimation_plots_tables(save=save)
    create_prediction_plots_tables(save=save, months=months)
...
def create_estimation_plots_tables(save=False):
    ...
    pte.table_estim_sii_all_months(save=save, month_to_priors=month_to_priors)
...
def create_prediction_plots_tables(save=False, months=None):

    rolling_predictions.rolling_predict()

    ptp.plot_sii_end_years(save=save, months=months)
    plt.close("all")

    ptp.plot_and_table_sii_predict_mod_sep(save=save, months=months)
    plt.close("all")
    ...
    ptp.plot_fyzi_and_ranges_table_sii_blend(save=save, months=months)
    plt.close("all")

    # use orig data for this -- no need to update
    ptp.plot_mean_compare_halves_sibt(save=save)
    ...
    ptp.prediction_plot_sep_sii_blend(save=save)
    plt.close("all")

    ptp.table_sii_predict(save=save, months=months)
    ptp.table_sibt_blend_ranges(save=save)
    ptp.tables_predict_from_precompute(save=save, months=months)
